...
class Registry {
  ...
  add(name, component) {
    ...
    this.errors = {
      ...this.errors,
      [name]: component.getErrorObject().hasErrors,
    };
    return this;
  }
  ...
  remove(name) {
    ...
    if (
      ...
      this.registry.get(name).keepModelData !== "inherit"
    ) {
  ...
  map(callback) {
    ...
    this.registry.forEach((component, field) =>
      Object.assign(value, { [field]: callback(component, field) })
    );
    return value;
  }
  ...
  register(field, component) {
    ...
    if (this.registry.has(field)) {
      ...
      this.ctx.$nextTick(() =>
        !this.registry.has(field) ? this.register(field, component) : false
      );
      return false;
    }
  ...
  return {
    ...
    initialValues() {
      if (
        ...
        this.isGrouping &&
        typeof this.context.model[this.index] === "object"
      ) {
    ...
    mergedGroupErrors() {
      ...
      return Object.keys(this.mergedFieldErrors)
        ...
        .reduce((groupErrorsByRoot, k) => {
          ...
          Object.assign(groupErrorsByRoot[rootField], {
            [groupKey]: this.mergedFieldErrors[k],
          });
...
export function useRegistryMethods(without = []) {
  const methods = {
    ...
    valueDeps(callerCmp) {
      return Object.keys(this.proxy).reduce(
        (o, k) =>
        ...
        this.deps
          ...
          .forEach(
            (field) =>
              ...
              this.registry.get(field).performValidation()
          );
      }
    },
    hasValidationErrors() {
      return Promise.all(
        this.registry.reduce((resolvers, cmp, name) => {
    ...
    addErrorObserver(observer) {
      if (
        !this.errorObservers.find((obs) => observer.callback === obs.callback)
      ) {
        ...
        if (observer.type === "form") {
          ...
          observer.type === "group" &&
          has(this.mergedGroupErrors, observer.field)
        ) {
    ...
    removeErrorObserver(observer) {
      this.errorObservers = this.errorObservers.filter(
        (obs) => obs.callback !== observer
      );
    },
  };
  ...
  const p = Object.keys(providers)
    ...
    .reduce(
      (useProviders, provider) =>
        Object.assign(useProviders, { [provider]: providers[provider] }),
      {}
    );
  return p;
}
